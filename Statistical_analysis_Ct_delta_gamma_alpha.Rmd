...
The median and range of Ct values for each clade in the `limited_clade_collection_diagnostics` table are as follows:
...
The median and range of Ct values for each `clade` - `order_priority` combination in the `limited_clade_collection_diagnostics` table are as follows:
...
selected_clades_lineage_table <- selected_clades_lineage_table %>%
  ...
  select(testkit_id, sample_id, collection_period, order_priority, collection_date, dhec_accession, genbank_accession, gisaid_epi_isl, clade, lineage) %>%
  arrange(collection_period, order_priority, sample_id)
...
selected_clades_lineage_table %>%
  ...
  write_csv("supplementary_table_1.csv")
```
...
The median and range of Ct values for each clade in the `limited_clade_collection_diagnostics_surveillance` table are as follows:
...
The median and range of Ct values for each clade in the `limited_clade_collection_diagnostics_symptomatic` table are as follows:
